...
type StatusFilter = "default" | "all" | ArticleStatus;
...
export default function KnowledgeBaseManager({
  ...
  const filtered = useMemo(() => items.filter((article) => {
    ...
    // Par défaut, les archives restent masquées. Le toggle « Tous » les inclut.
    const matchesStatus = status === "default"
      ...
      <section className="overflow-hidden rounded-2xl border border-outline-variant/20 bg-white shadow-sm">
        <div className="border-b border-outline-variant/20 p-5">
          <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            ...
            <Button type="button" onClick={() => setDrawer({ article: null, form: emptyForm() })}>
          ...
          <div className="mt-4 flex flex-wrap items-center gap-3">
            ...
            <div className="flex flex-wrap gap-2" aria-label="Filtrer par statut">
              {(["all", "draft", "published", "archived"] as StatusFilter[]).map((value) => (
                <button key={value} type="button" onClick={() => { setStatus(value); setPage(1); }} className={`rounded-full px-3 py-1.5 text-xs border font-semibold transition-colors ${status === value ? "bg-primary-container text-on-primary" : "bg-white text-on-surface-variant hover:bg-primary-container"}`}>
          ...
          <table className="w-full min-w-[1060px] text-left">
            ...
            <tbody className="divide-y divide-outline-variant/15">
              {visibleArticles.map((article) => (
                <tr key={article.id} className="transition-colors hover:bg-surface-container-low/70">
                  ...
                  <td className="px-5 py-4"><div className="flex justify-end gap-1"><button type="button" onClick={() => setDrawer({ article, form: { title: article.title, content: article.content, category: article.category, keywords: article.keywords } })} className="inline-flex size-8 items-center justify-center rounded-lg text-on-surface-variant hover:bg-primary-container hover:text-on-primary-container" aria-label={`Modifier ${article.title}`}><FilePenLine size={16} /></button>{article.status === "draft" && <button type="button" onClick={() => applyStatus(article, "published")} className="inline-flex size-8 items-center justify-center rounded-lg text-teal-700 hover:bg-teal-100" aria-label={`Publier ${article.title}`}><Send size={16} /></button>}{article.status !== "archived" && <button type="button" onClick={() => applyStatus(article, "archived")} className="inline-flex size-8 items-center justify-center rounded-lg text-on-surface-variant hover:bg-stone-200" aria-label={`Archiver ${article.title}`}><Archive size={16} /></button>}</div></td>
                </tr>
              ))}
...
function ArticleDrawer({ drawer, categories, onClose, onSave }: { drawer: { article: KnowledgeArticle | null; form: ArticleForm }; categories: string[]; onClose: () => void; onSave: (form: ArticleForm) => void }) {
