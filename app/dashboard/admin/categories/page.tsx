    ...
    <CategoriesPageTransition>
      ...
      <div className="mb-7 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <Breadcrumb className="mb-5">
            <BreadcrumbList>
              <BreadcrumbItem><BreadcrumbLink asChild><Link href="/dashboard/admin">Admin</Link></BreadcrumbLink></BreadcrumbItem>
              <BreadcrumbSeparator />
              <BreadcrumbItem><BreadcrumbLink asChild><Link href="/dashboard/admin/tickets">Tickets</Link></BreadcrumbLink></BreadcrumbItem>
              <BreadcrumbSeparator />
              <BreadcrumbItem><BreadcrumbPage>Catégories</BreadcrumbPage></BreadcrumbItem>
            </BreadcrumbList>
          </Breadcrumb>
          <h1 className="mt-2 text-2xl font-bold text-on-surface">Gestion des catégories</h1>
          <p className="mt-1 text-base text-on-surface-variant">Organisez les catégories utilisées pour qualifier les demandes de support.</p>
        </div>
        <NewTicketCategoryDialog />
      </div>
      <section className="mb-8 grid grid-cols-1 gap-4 md:grid-cols-3">
        <article className="flex items-center gap-4 rounded-xl border border-outline-variant/20 bg-white p-5 shadow-lg">
          ...
          <div>
            <p className="text-sm font-medium text-on-surface-variant">Total catégories</p>
            <p className="text-2xl font-bold text-on-surface">{ticketCategories.length}</p>
          </div>
        </article>
        <article className="flex items-center gap-4 rounded-xl border border-outline-variant/20 bg-white p-5 shadow-lg">
          ...
          <div>
            <p className="text-sm font-medium text-on-surface-variant">Activité du système</p>
            <p className="font-semibold text-on-surface">Base de données synchronisée</p>
          </div><Activity className="ml-auto text-emerald-700" size={20} />
        </article>
      </section>
